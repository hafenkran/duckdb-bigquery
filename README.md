...
| `bq_read_prefetch_queue_size`             | [EXPERIMENTAL] - Maximum number of decoded BigQuery Storage Read Arrow batches to prefetch per active read worker. Set to `0` to disable read prefetching.                                          | `2`     |
| `bq_read_prefetch_max_memory`             | [EXPERIMENTAL] - Approximate shared memory budget in bytes for queued BigQuery Storage Read prefetch batches. Set to `0` to disable the byte budget.                                                | `67108864` |

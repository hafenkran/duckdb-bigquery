...
| `bq_enable_aggregate_pushdown`         | Whether to rewrite supported ungrouped and grouped BigQuery aggregates, including `COUNT(DISTINCT column)`, arithmetic arguments, numeric casts, and compatible scalar `WHERE` expressions, to BigQuery query jobs instead of scanning all source rows through the Storage Read API. | `true`  |

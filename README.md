...
| `bq_enable_aggregate_pushdown`         | Whether to rewrite supported ungrouped and grouped BigQuery aggregates, including compatible `COUNT(DISTINCT expression)`, arithmetic arguments with `SIN` and FLOAT/DOUBLE modulo, `CAST`, `TRY_CAST`, `LOWER`, `ABS`, and LIST/STRUCT-constructor-to-`VARCHAR` casts in compatible aggregate arguments, recursive arithmetic `GROUP BY` expressions with `ABS`, division, integer/decimal modulo, FLOAT/DOUBLE modulo, scalar-to-`VARCHAR` casts, numeric and temporal `CAST`/`TRY_CAST`, `GROUP BY LOWER`, and LIST/STRUCT-constructor-to-`VARCHAR` casts, numeric and temporal `CAST`/`TRY_CAST` in compatible filters, recursive LIST/STRUCT-constructor-to-`VARCHAR` casts, `SIN`, and FLOAT/DOUBLE modulo in compatible filters, and scalar `WHERE` expressions such as `LOWER` and `ABS`, to BigQuery query jobs instead of scanning all source rows through the Storage Read API. | `true`  |

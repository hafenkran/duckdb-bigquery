...
| `bq_enable_aggregate_pushdown`         | Whether to rewrite supported ungrouped and grouped BigQuery aggregates, including `COUNT(DISTINCT column)`, to BigQuery query jobs instead of scanning all source rows through the Storage Read API.                                                  | `true`  |
